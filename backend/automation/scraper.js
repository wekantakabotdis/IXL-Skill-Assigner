...
async function scrapeSkills(page, gradeLevel = '8') {
  try {
    const gradeUrlMap = {
      'pre-k': 'preschool',
    ...
    const url = `https://www.ixl.com/math/grade-${urlGrade}`;
    
    console.log(`Navigating to grade ${gradeLevel} skills page: ${url}`);
    ...
    const skills = await page.evaluate((grade) => {
      const results = [];
